...
5. **One-off cleanups** — Best-effort `DELETE` for deprecated rows (for example legacy `room_general`) wrapped in try/catch so reruns do not fail.
...
- **Default admin / moderator** — Handled in **`server/utils/seedAdmin.js`** from **`server/index.js`** (env-gated; see root **README** for `SEED_ADMIN`, `ADMIN_EMAIL`, production behavior).
