...
/**
 * Default moderator account (separate login from admin).
 * Skipped when SEED_ADMIN=0 (same as admin — no demo accounts) or SEED_MODERATOR=0.
 * In production, runs only when SEED_ADMIN=1.
 */
export async function seedDefaultModerator() {
  if (process.env.SEED_ADMIN === '0' || process.env.SEED_MODERATOR === '0') return

  const isProd = process.env.NODE_ENV === 'production'
  if (isProd && process.env.SEED_ADMIN !== '1') return

  const email = String(process.env.MODERATOR_EMAIL || 'mod@connectly.local').trim().toLowerCase()
  const password = process.env.MODERATOR_PASSWORD || 'ConnectlyMod2026!'

  if (!email) {
    console.warn('[seed] MODERATOR_EMAIL empty; skipping moderator seed')
    return
  }
  if (password.length < 6) {
    console.warn('[seed] MODERATOR_PASSWORD must be at least 6 characters; skipping moderator seed')
    return
  }
  ...
  const id = `user_mod_${crypto.randomBytes(8).toString('hex')}`
  const hash = await bcrypt.hash(password, 10)
  const now = new Date().toISOString()
  ...
  console.log(
    `[seed] Default moderator created: ${email} (display: Moderator). Password: set via MODERATOR_PASSWORD or default — change for anything beyond local use.`,
  )
}
